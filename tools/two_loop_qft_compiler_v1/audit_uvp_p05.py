"""Post-execution integrity audit for UVP_P05."""
...
def main():
    schema = json.loads((HERE / "uvp_p05_evidence_schema.json").read_text(
        encoding="utf-8"))
    evidence = json.loads((HERE / "uvp_p05_evidence.json").read_text(
        encoding="utf-8"))
    primary = json.loads((HERE / "uvp_p05_primary.json").read_text(
        encoding="utf-8"))
    replay = json.loads((HERE / "uvp_p05_independent_replay.json").read_text(
        encoding="utf-8"))
    uv_ir = json.loads((HERE / "uvp_p05_uv_ir_provenance.json").read_text(
        encoding="utf-8"))
    ...
    assert primary["rank4"]["pairings"] == replay["rank4"]["pairings"]
    assert primary["rank6"]["pairings"] == replay["rank6"]["pairings"]
    assert all(value == "0" for value in primary["recursive_contractions"].values())
    assert all(value == "0" for value in replay["recursive_contractions"].values())

    row = ledger["tests"][4]
    assert row["test_id"] == "UVP_P05" and row["status"] == "PASS"
    ...
    print("UVP_P05_EVIDENCE_AUDIT_PASS")
