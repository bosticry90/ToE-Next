"""Exact fieldwise-rephasing rank for the closed scalar invariant basis."""
...
DELTA = (2, 2, 0)      # Sigma Sigma phi phi
...
def main():
    assert all(dot(row, PQ) == 0 for row in (CHI4, CHI6, ETA1, KAPPA, DELTA))
    assert tuple(CHI4[i] + CHI6[i] for i in range(3)) == tuple(2 * x for x in ETA1)
    assert tuple(CHI4[i] + CHI6[i] for i in range(3)) == DELTA
    ...
    print("Sigma^2 phi^2 gives a third generic coupling-phase invariant: arg(delta)-arg(chi4)-arg(chi6).")
    print("Five complex coefficients minus two available field rephasings = three relative phases.")
