...
The [next admission record](BABU_KHAN_PS_RUNNING_AND_MI_MATCHING.md) finds that published work already supplies the unique Pati-Salam operator, gauge-leading-log anomalous exponents, and a simplified tree-level Standard Model projection. Its [calculation](../calculations/bk_ps_bnv_running_mi_matching/RESULT.md) resolved `PASS`: the Babu-Khan beta ledger, analytic gauge-running factor, full flavor structure, right-handed-neutrino retention, and post-running `M_I` projector agree in independent exact replays. The later [`H_T` threshold audit](BABU_KHAN_HT_THRESHOLD_AUDIT.md) corrected the beta-ledger sextet's identity to intermediate `Sigma_1`; `H_T` remains a high-scale gauge-threshold field.

The [downstream admission audit](BABU_KHAN_SMEFT_N_THRESHOLD_AUDIT.md) finds mature one-loop BNV SMEFT-plus-`N` running machinery but no frozen source-model neutrino spectrum or turnkey physical `Q_qqdN` threshold carry-forward. The proposed seam from `M_I` to the first physical singlet-neutrino threshold is therefore not admitted. A compatible numerical fermion fit, mass-basis rotation, threshold order, and dimension-seven disposition must be established first. No running below `M_I` or phenomenology has been performed.

The [fit provenance audit](BABU_KHAN_FERMION_FIT_PROVENANCE_AUDIT.md) located the Babu-Khan paper's quoted published point and other numerical fits, but their assumptions do not allow unchanged adoption as this model's `M_I` initial condition. No benchmark or Takagi spectrum was selected or calculated. A new source-model-specific re-fit or alternative-source comparison would require a separate scientific decision.
