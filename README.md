...
The [physical lower-boundary background-field continuation](calculations/canonical_so10_lower_bfm/RESULT.md) now verifies the nine `PS/SM` broken vector--Goldstone--ghost quadratic mass pairings in a partial `R_zeta` gauge. The full one-loop gauge-kinetic coefficient and its finite matching terms remain uncomputed, so the lower boundary remains `LOWER_BOUNDARY_MATCHING_BLOCKED`; upper-boundary matching and gauge-scale fitting were not started.
