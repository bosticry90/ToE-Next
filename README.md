...
A separate [canonical source-derived scalar reconstruction](calculations/canonical_so10_scalar_reconstruction/RECORD.md) is open **in the exploratory lane**. It freezes a coherent charge/kinetic starting convention and a gate-ordered scalar search protocol, while refusing to call the printed source potential a complete canonical action. Exact PQ arithmetic exposes an additional as-printed conflict between the source charge table and its Yukawa/`eta_1` monomials. The [invariant-gate calculation](calculations/canonical_so10_scalar_reconstruction/RESULT.md) establishes that the corrected PQ-neutral `eta_1` field multiset has one nonzero SO(10) invariant, and counts all 34 representation-theory scalar singlet slots. It also finds a PQ-allowed `Phi phi phi S*` slot omitted from the printed potential. Explicit tensor-basis rank, Hermitian normalization, and the parent action remain open: `INVARIANT_BASIS_UNRESOLVED`. No scalar benchmark scan, flavor fit, or new active recovery seam has begun.
