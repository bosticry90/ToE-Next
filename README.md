...
A separate [canonical source-derived scalar reconstruction](calculations/canonical_so10_scalar_reconstruction/PARENT_ACTION_V1.md) remains **in the exploratory lane**, distinct from the printed Babu--Khan scalar model. Its complete 34-direction renormalizable invariant action, intended gauge stabilizer, exact tadpoles, and [full 35-block scalar Hessian](calculations/canonical_so10_full_hessian/RESULT.md) have passed their bounded gates. A [new scalar benchmark](calculations/canonical_so10_scalar_benchmark/RESULT.md) establishes one locally positive high-scale stationary point with exactly 33 gauge plus one PQ zero modes. A separately tuned point has the correct quadratic light-doublet nullity, but its [heavy-relaxed Higgs quartic is negative](calculations/canonical_so10_light_higgs_quartic/RESULT.md), so that tuned point is not a local minimum. These are original scalar-only results; global stability, loop-level perturbativity, fermion flavor, scalar-mediated baryon-number violation, and phenomenological GUT viability remain unestablished. No published Babu--Khan scalar benchmark or scalar threshold has been inherited.
