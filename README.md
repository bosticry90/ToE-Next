...
A separate [canonical source-derived scalar reconstruction](calculations/canonical_so10_scalar_reconstruction/RECORD.md) is now open **in the exploratory lane**. It freezes a coherent charge/kinetic starting convention and a gate-ordered scalar search protocol, while refusing to call the printed source potential a complete canonical action. Exact PQ arithmetic exposes an additional as-printed conflict between the source charge table and its Yukawa/`eta_1` monomials. The independent invariant-basis/action-definition gate has not passed, so the current outcome is `CANONICAL_MODEL_UNDERDEFINED`; no scalar benchmark scan, flavor fit, or new active recovery seam has begun.
